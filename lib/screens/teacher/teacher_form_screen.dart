...
class TeacherFormScreen extends StatefulWidget {
  ...
  const TeacherFormScreen(
      {Key key, @required this.title, this.isEdit, this.teacherDomain})
      : super(key: key);
...
class _TeacherFormScreenState extends State<TeacherFormScreen> {
  ...
  void initState() {
    ...
    if (widget.isEdit) {
      ...
      valueMapel = widget.teacherDomain.mata_pelajaran;
      birthDate = widget.teacherDomain.birthDay;
    }
  }
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            SizedBox(height: 10),
            Text('Mata Pelajaran', style: TextStyle(color: Colors.blue)),
            Column(
                children: TeacherDomain.mapelList
                    .map((e) => CustomCheckBox(
                          text: e,
                          value: valueMapel.contains(e),
                          onChanged: (value) {
                            setState(() {
                              if (valueMapel.contains(e)) {
                                valueMapel.remove(e);
                              } else {
                                valueMapel.add(e);
                              }
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          if(_formKey.currentState.validate()){
            ...
            teacherDomain.birthDay = birthDate;
            teacherDomain.mata_pelajaran = valueMapel;
            ...
            if(widget.isEdit){
              AppServices.getTeacherService.updateTeacher(widget.teacherDomain.idGuru, teacherDomain).then((value){
                Navigator.pop(context);
              }).catchError((error){
                // ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(error.toString())));
              });
            }else{
              AppServices.getTeacherService.createTeacher(teacherDomain).then((value){
                Navigator.pop(context);
              }).catchError((error){
                print(error);
                // ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(error.toString())));
